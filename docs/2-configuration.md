...
| `WG_INACTIVE_DURATION`               | `--inactive-duration`               | `inactiveDuration`             |          | `4380h`                                      | The duration after which inactive devices are automatically deleted, if automatic deletion is enabled. A device is inactive if it has not been connected to the server for longer than the inactive duration. The duration format is the go duration string format |
